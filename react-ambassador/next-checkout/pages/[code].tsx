...
export default function Home() {
      ...
      try {
        (async () => {
          const { data } = await axios.get(
            `${process.env.NEXT_PUBLIC_NODE_AMBASSADOR}/api/checkout/links/${code}`
          );
  ...
  const submit = async (e: SyntheticEvent) => {
    e.preventDefault();
    if (quantities.some((qty) => qty.quantity === 0)) {
      alert("Please select quantities");
      return;
    }

    const { data } = await axios.post(
      `${process.env.NEXT_PUBLIC_NODE_AMBASSADOR}/api/checkout/orders`,
      {
        first_name,
        last_name,
        email,
